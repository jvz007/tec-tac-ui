...
export function getCoreCurrentSession() {
  return apiFetch('/api/tfd/session/current/')
}

export function getCoreSessionPolicy() {
  return apiFetch('/api/tfd/session/policy/')
}

export function updateCoreSessionPolicy(policy) {
  return apiFetch('/api/tfd/session/policy/', {
    method: 'PUT',
...
export function listCoreSessions(username = '') {
  const query = new URLSearchParams()
  if (username) query.set('username', username)
  const suffix = query.toString() ? `?${query.toString()}` : ''
  return apiFetch(`/api/tfd/session/sessions/${suffix}`)
}

export function revokeCoreSession(sessionId, reason = 'administrator-request') {
  return apiFetch(`/api/tfd/session/sessions/${encodeURIComponent(sessionId)}/revoke/`, {
    method: 'POST',
    body: JSON.stringify({ reason }),
  })
}
...
export function getCoreSessionDiagnostics() {
  return apiFetch('/api/tfd/session/diagnostics/')
}
