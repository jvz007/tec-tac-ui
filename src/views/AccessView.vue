...
<template>
  <section>
    ...
    <div class="grid g2">
      <article class="card"><div class="cardhead"><h3>Tactical identity</h3><span class="pill ok">verified</span></div><dl class="kvlist"><dt>Username</dt><dd class="mono">{{ state.context.user?.username }}</dd><dt>Display name</dt><dd>{{ state.context.user?.display_name }}</dd><dt>Role</dt><dd>{{ state.context.user?.role || '—' }}</dd><dt>Role ID</dt><dd class="mono">{{ state.context.user?.role_id ?? '—' }}</dd><dt>Superuser</dt><dd>{{ state.contextSource === 'backend' ? (state.context.user?.superuser ? 'yes' : 'no') : 'not resolved by shell' }}</dd><dt>Auth proof</dt><dd class="mono">{{ state.authProof || '—' }}</dd><dt>Context source</dt><dd class="mono">{{ state.contextSource }}</dd></dl></article>
