  ...
  <div v-if="modelValue" class="modal-backdrop quick-actions-backdrop" @click.self="close">
    <section class="modal-panel quick-actions-dialog" role="dialog" aria-modal="true" aria-labelledby="quick-actions-title">
      <div class="cardhead quick-actions-dialog-head">
        <div><span class="eyebrow">PERSONAL WORKSPACE</span><h3 id="quick-actions-title">Quick Actions</h3><p>Pin frequently used actions registered by Tec-Tac modules. The owning module controls what the action does.</p></div>
      ...
      <div class="section-divider quick-section-divider">Pinned actions</div>
      <div v-if="pins.length" class="quick-manage-list">
        <div v-for="(pin,index) in pins" :key="pin.id" class="quick-manage-row">
          ...
          <div class="quick-manage-copy"><b>{{ pin.label }}</b><span class="mono">{{ pin.action_id }}</span><small v-if="pin.state?.enabled === false" class="dangertext">{{ pin.state.reason }}</small></div>
          <span class="pill" :class="pin.state?.enabled === false ? 'warn' : 'ok'">ACTION</span>
      ...
      <div v-else class="callout mono">No module actions pinned yet.</div>

      <div class="section-divider quick-section-divider">Add action</div>
      <label class="search quick-actions-search"><span class="sr-only">Search registered actions</span><input v-model="query" placeholder="Search registered module actions…"></label>
      ...
      <div class="quick-catalog">
        ...
        <div v-else class="callout mono">No additional registered actions match the current search.</div>
      </div>
