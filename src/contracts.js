import { apiBase, apiFetch, tacticalToken } from './api'
...
export async function downloadDeveloperContracts(format='md'){
  ...
  const response=await fetch(`${base}/api/tfd/contracts/export/?format=${encodeURIComponent(format)}`,{
    headers:{Accept:format==='txt'?'text/plain':'text/markdown',Authorization:`Token ${token}`},
    credentials:'include',cache:'no-store'
  })
  if(!response.ok){let msg=`Contract export failed: ${response.status}`;try{const p=await response.json();msg=p.detail||p.error||msg}catch{};throw new Error(msg)}
